...
func TestCameraCreationContinuesAsynchronously(t *testing.T) {
	...
	server := NewServer(
		...
		media.New(mediaServer.URL, "rtsp://media", mediaServer.URL, mediaServer.URL, filepath.Join(directory, "recordings")),
		nil,
		nil,
		nil,
		NewHub(),
...
func TestCameraCreationPersistsProbeFailure(t *testing.T) {
	...
	repository := camera.NewRepository(database, vault)
	server := NewServer(authManager, repository, camera.NewONVIFClient(), media.New("http://127.0.0.1:1", "rtsp://127.0.0.1:1", "http://127.0.0.1:1", "http://127.0.0.1:1", directory), nil, nil, nil, NewHub(), slog.New(slog.NewTextHandler(io.Discard, nil)))
