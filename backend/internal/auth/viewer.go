...
func (m *Manager) issueViewer(ctx context.Context) (PairResponse, error) {
	...
	id := uuid.NewString()
	_, err = m.store.DB.ExecContext(ctx, `INSERT INTO viewer_sessions(id,token_hash,expires_at,created_at) VALUES(?,?,?,?)`, id, appcrypto.Hash(token), now.Add(12*time.Hour).Format(time.RFC3339Nano), now.Format(time.RFC3339Nano))
