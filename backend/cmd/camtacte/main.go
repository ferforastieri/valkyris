...
func main() {
	...
	go func() {
		logger.Info("Camtacte listening", "address", cfg.Listen, "version", version)
...
func ensureCertificate(certPath, keyPath string) error {
	...
	template := x509.Certificate{SerialNumber: serial, Subject: pkix.Name{CommonName: "camtacte.local", Organization: []string{"Camtacte local installation"}}, NotBefore: time.Now().Add(-time.Hour), NotAfter: time.Now().AddDate(10, 0, 0), KeyUsage: x509.KeyUsageDigitalSignature | x509.KeyUsageKeyEncipherment, ExtKeyUsage: []x509.ExtKeyUsage{x509.ExtKeyUsageServerAuth}, DNSNames: []string{"camtacte.local", "localhost"}, IPAddresses: []net.IP{net.ParseIP("127.0.0.1"), net.ParseIP("::1")}, BasicConstraintsValid: true}
