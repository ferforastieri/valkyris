package com.ferforastieri.camtacte.core.security
...
data class Session(val baseUrl:String,val token:String,val fingerprint:String="")
...
@Singleton class SessionStore @Inject constructor(@ApplicationContext context:Context) {
    ...
    private val keyAlias="camtacte_session_v1"
    fun get():Session? { val base=prefs.getString("base",null)?:return null;val token=decrypt(prefs.getString("token",null)?:return null);return Session(base,token,prefs.getString("fingerprint","").orEmpty()) }
    fun save(session:Session){prefs.edit().putString("base",session.baseUrl.trimEnd('/')).putString("token",encrypt(session.token)).putString("fingerprint",session.fingerprint).apply()}
