...
@Serializable data class LightCapabilities(val brightness:Boolean=true,val color:Boolean=true,val colorTemperature:Boolean=true)
@Serializable data class LightColor(val hue:Int=0,val saturation:Int=100,val value:Int=100)
@Serializable data class LightState(val online:Boolean=false,val power:Boolean=false,val mode:String="white",val brightness:Int=100,val temperatureKelvin:Int=4000,val color:LightColor=LightColor(),val updatedAt:String="")
@Serializable data class LightDevice(val id:String,val name:String,val room:String="",val deviceId:String="",val protocolVersion:Double=3.3,val capabilities:LightCapabilities=LightCapabilities(),val setupStatus:String="pending",val setupError:String="",val enabled:Boolean=true,val lastSeenAt:String?=null,val state:LightState=LightState())
@Serializable data class CreateLightRequest(val name:String,val room:String="",val deviceId:String,val localKey:String,val ip:String="",val protocolVersion:Double=3.3)
@Serializable data class UpdateLightRequest(val name:String,val room:String="",val localKey:String="",val ip:String="",val protocolVersion:Double=3.3,val enabled:Boolean=true)
@Serializable data class LightStatePatch(val power:Boolean?=null,val mode:String?=null,val brightness:Int?=null,val temperatureKelvin:Int?=null,val color:LightColor?=null)
...
@Serializable data class RuleActions(val record:Boolean=true,val notify:Boolean=true,val alarm:Boolean=false,val recipientUserIds:List<String>?=null,val alerts:AlertPresentation=AlertPresentation())
...
@OptIn(kotlinx.serialization.ExperimentalSerializationApi::class)
@Serializable data class RuleActionsRequest(val record:Boolean,val notify:Boolean,val alarm:Boolean,val recipientUserIds:List<String>?=null,@kotlinx.serialization.EncodeDefault val alerts:AlertPresentation=AlertPresentation())
