...
@Composable private fun EventCard(event:EventEntity,onOpen:()->Unit,onAck:()->Unit){Card(onClick=onOpen,modifier=Modifier.fillMaxWidth(),colors=CardDefaults.cardColors(containerColor=MaterialTheme.colorScheme.surface),elevation=CardDefaults.cardElevation(defaultElevation=4.dp)){Row(Modifier.padding(14.dp),verticalAlignment=Alignment.CenterVertically){Box(Modifier.size(44.dp).background(if(event.acknowledgedAt==null)MaterialTheme.colorScheme.error.copy(alpha=.12f)else MaterialTheme.colorScheme.secondary.copy(alpha=.15f),MaterialTheme.shapes.small)){Box(Modifier.size(9.dp).background(if(event.acknowledgedAt==null)MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.secondary,CircleShape).align(Alignment.Center))};Spacer(Modifier.width(12.dp));Column(Modifier.weight(1f)){Text(event.type.replace('_',' ').replaceFirstChar{it.uppercase()},fontWeight=FontWeight.SemiBold);Text(formatTime(event.occurredAt),style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant);Text("${(event.confidence*100).toInt()}% confidence",style=MaterialTheme.typography.labelSmall,color=MaterialTheme.colorScheme.onSurfaceVariant)};if(event.acknowledgedAt==null)IconButton(onAck){Icon(Icons.Rounded.Check,stringResource(R.string.acknowledge))}}}}
