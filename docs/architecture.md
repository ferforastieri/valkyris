...
tokens are stored only as SHA-256 hashes. The local setup page requires the
installation secret generated by `install.sh`; after authorization it stores
that secret in a secure, HTTP-only cookie. Every displayed QR contains a fresh,
short-lived, single-use pairing code plus the pinned certificate fingerprint.
The page and QR renderer are embedded in the Go binary and make no external
requests.
